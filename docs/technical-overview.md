# StayFlow — Technical Overview

> Engineering documentation for StayFlow. For the product overview, live demo, and screenshots, see the [root README](../README.md).
...
> The live password is shown in the [root README's "Try It Live" section](../README.md#try-it-live) — this is a portfolio project with seeded demo accounts, not real user data. **Rotate before any production use** via the password-reset flow or `server/scripts/reset-test-passwords.js` (set `TEST_PASSWORD`).
