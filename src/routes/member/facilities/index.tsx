...
const categories: (FacilityCategory | 'All')[] = [
  'All',
  'Wellness',
  'Sports',
  'Entertainment',
...
function FacilitiesList() {
  ...
  const load = React.useCallback((residentId?: string) => {
    ...
    Promise.all([
      getFacilities(),
      residentId ? getMyBookings(residentId) : Promise.resolve([]),
    ])
      .then(([f, b]) => {
        if (!active) return
        setFacilities(f)
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="border-border bg-surface">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Your slot at {b.facilityName ?? 'this facility'} on{' '}
                          {b.date.slice(0, 10)} · {b.timeSlot} will be released.
