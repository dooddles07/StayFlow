...
function DiningList() {
  ...
  const load = React.useCallback((residentId?: string) => {
    ...
    Promise.all([
      getRestaurants(),
      residentId ? getMyReservations(residentId) : Promise.resolve([]),
    ])
      .then(([r, res]) => {
        if (!active) return
        setRestaurants(r)
  ...
  const history = [...reservations]
    ...
    .sort(
      (a, b) =>
        byHistorySort(historySort)(a.date, b.date) ||
        byHistorySort(historySort)(a.time, b.time),
    )

  // Cancelled reads as cancelled; a guest who arrived or a past reservation reads as completed.
  const outcome = (r: ReservationView) =>
      ...
      {status === 'ready' && upcoming.length > 0 && (
        <div className="mb-8">
          <SectionHeader
            ...
            description="Upcoming requests and confirmations"
          />
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-text">
                    {r.date.slice(0, 10)} at {r.time} · Party of {r.partySize} ·{' '}
                    {r.seating}
                  </p>
                </div>
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button
                        ...
                        className="border-border text-rose-400 hover:bg-rose-500/10 hover:text-rose-400"
                      >
                    ...
                    <AlertDialogContent className="border-border bg-surface">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Your table at {r.restaurantName ?? 'this restaurant'}{' '}
                          on {r.date.slice(0, 10)} at {r.time} will be released.
                ...
                {history.map((r) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-text">
                        {r.date.slice(0, 10)} at {r.time} · Party of{' '}
                        {r.partySize} · {r.seating}
                      </p>
                    </div>
