    ...
    <AlertDialog>
      ...
      <AlertDialogContent className="border-border bg-surface">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {booking.residentName ?? 'This resident'}'s{' '}
            {booking.facilityName ?? 'facility'} booking for{' '}
            {booking.date.slice(0, 10)} · {booking.timeSlot} will be declined.
...
function BookingsPage() {
  ...
  async function updateStatus(id: string, next: BookingStatus) {
    ...
    try {
      ...
      toast.success(
        next === 'confirmed' ? 'Booking approved' : 'Booking rejected',
      )
    } catch (err) {
        ...
        <TabsList className="bg-surface">
          {statusFilters.map((s) => (
            <TabsTrigger
              key={s}
              value={s}
              className="capitalize data-[state=active]:bg-accent-indigo/20 data-[state=active]:text-accent-gold"
            >
              {s}
            </TabsTrigger>
          ))}
      ...
      {status === 'loading' ? (
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-2 text-xs text-muted-text">
                    {b.date.slice(0, 10)} · {b.timeSlot} · Party of{' '}
                    {b.partySize}
                  </p>
                  {b.status === 'pending' && (
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody className="divide-y divide-border bg-surface">
                {filtered.map((b, i) => {
                  const busy = busyIds.has(b.id)
                  return (
                    <AnimatedTableRow key={b.id} index={i}>
                      ...
                      <td className="px-4 py-3">
                        {b.status === 'pending' ? (
                          ...
                          <span className="block text-right text-xs text-muted-text/60">
                            —
                          </span>
                        )}
                      </td>
        ...
        <div className="space-y-6">
          {Object.entries(grouped).map(([date, items]) => (
            <div key={date}>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-accent-gold">
                {date}
              </p>
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-text">
                          {b.residentName} · Party of {b.partySize}
                        </p>
                      </div>
