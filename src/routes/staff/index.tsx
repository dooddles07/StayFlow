...
function StaffDashboard() {
  ...
  const todaysGuests = guests.filter(
    (g) => g.arrivalDate.slice(0, 10) === today,
  )
  const todaysBookings = bookings.filter(
    (b) => b.date.slice(0, 10) === today && b.status !== 'cancelled',
  )
  const todaysDining = reservations.filter(
    (d) => d.date.slice(0, 10) === today && d.status !== 'cancelled',
  )
  const pendingCount =
  ...
  const tasks = [
    ...bookings
      ...
      .map((b) => ({
        id: b.id,
        label: `Approve ${b.facilityName ?? 'facility'} booking`,
        href: '/staff/bookings',
      })),
    ...guests
      ...
      {status === 'error' ? (
        <div className="rounded-2xl border border-border bg-surface p-8 text-center">
          <p className="text-sm text-muted-text">
            We couldn't load today's overview right now.
          </p>
          <Button
            onClick={load}
        ...
        <>
          <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <KpiCard
              ...
              hint="Across bookings, dining & guests"
            />
          </div>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <SectionHeader
                ...
                description={`${scheduleRows.length} items`}
              />
                  ...
                  <table className="w-full min-w-[480px] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border bg-surface">
                      {scheduleRows.map((row, i) => (
                        <AnimatedTableRow key={row.id} index={i}>
                ...
                {alerts.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {alerts.map((f) => (
                      <div
                        key={f.id}
                        className="flex items-start gap-2.5 rounded-xl border border-accent-gold/25 bg-accent-gold/10 px-3.5 py-3"
                      >
