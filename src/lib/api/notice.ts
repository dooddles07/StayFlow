...
export const createNotice = (data: NoticeInput) =>
  api.post<Notice>('/notices', data).finally(bust)
export const updateNotice = (id: string, data: NoticeInput) =>
  api.put<Notice>(`/notices/${id}`, data).finally(bust)
export const deleteNotice = (id: string) =>
  api.del<void>(`/notices/${id}`).finally(bust)
